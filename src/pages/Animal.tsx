...
export const Animal = () => {
  ...
  const clickedAnimal = animals.find(
    (animal: IAnimal) => animal.id.toString() === params.id
  )!;

  let currentTime = new Date().getTime();

  //   animals.map((animal) => {
  //     if (currentTime - Date.parse(animal.lastFed) > 1080) {
  ...
  return (
    <>
      <div className="animal-detail-container">
        ...
        <button
          onClick={() => {
            feedAnimal(clickedAnimal);
          }}
          disabled={clickedAnimal.isFed === true}
        >
          Mata
        </button>
      </div>
